...
func init() {
	...
	// 连接数据库 此处第一个参数必须设置为“default”（因为我现在只有一个数据库），否则编译报错说：必须有一个注册DB的别名为 default
	orm.RegisterDataBase("default", "mysql", "root:root@tcp(127.0.0.1:3306)/lovehome?charset=utf8", 30)

	// 注册原型
	orm.RegisterModel(new(User), new(OrderHouse), new(Facility), new(HouseImage), new(Area), new(House))

	// 自动建表 第二个参数设置为false表示如果表已存在，不需要删除重建
